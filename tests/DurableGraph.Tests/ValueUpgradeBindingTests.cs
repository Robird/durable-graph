...
public sealed class ValueUpgradeBindingTests {
    ...
    public void NestedGenericRulesUseLocalScopesAndFreshOwnerContextsAcrossObjectsAndEdges() {
        ...
        foreach (uint id in new uint[] { 11, 22 }) {
            var result = context.Normalize<Box3<Pair2<Point2>>>(new(id, source,
        ...
        Assert.Equal(new uint[] { 11, 11, 11, 11, 11, 22, 22, 22, 22, 22 }, Calls.Select(item => item.Context.ObjectId));
    ...
    public void KeepExactRequiresExplicitOptInAndDoesNotOverrideAnExplicitSameSchemaRule() {
        ...
        Assert.Throws<InvalidDataException>(() => absent.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target));
        Assert.Empty(Calls);
        ...
        Assert.Equal(5, identity.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target).Value);
        ...
        Assert.Equal(6, selected.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target).Value);
    ...
    public void MissingUnusedDependencyAndMissingLaterEdgeDependencyPrecedeEveryOwnerCallback() {
        ...
        DurableSchema source = BoxSchema(1, new(1, TypeTag.Int32)), target = BoxSchema(2, new(1, TypeTag.Int32));
        Assert.Throws<InvalidDataException>(() => unused.Normalize<Box2<int>>(new(1, source, new Box1<int>(3)), target));
        Assert.Empty(Calls);
        ...
        Assert.Throws<InvalidDataException>(() => later.Normalize<Box3<int>>(new(1, source, new Box1<int>(3)), target));
        Assert.Empty(Calls);
        ...
        Assert.Throws<InvalidDataException>(() => nested.Normalize<Box2<Pair2<Point2>>>(
            new(1, BoxSchema(1, pair), new Box1<Pair2<Point2>>(default)), BoxSchema(2, pair)));
        Assert.Empty(Calls);
    }
    ...
    public void WrongCompleteSchemaSignatureConstraintAndAmbiguityNeverBecomeKeepExact() {
        ...
        foreach (StateValueUpgradeProvider provider in new[] { wrongSchema, wrongSignature }) {
            ...
            Assert.Throws<InvalidDataException>(() => context.Normalize<Box2<Point2>>(new(1, source, new Box1<Point2>(new(3))), target));
            Assert.Empty(Calls);
        }
        ...
        Assert.Throws<InvalidDataException>(() => constrained.Normalize<Box2<int>>(
            new(1, BoxSchema(1, new(1, TypeTag.Int32)), new Box1<int>(3)), BoxSchema(2, new(1, TypeTag.Int32))));
        Assert.Empty(Calls);
        ...
        Assert.Throws<InvalidDataException>(() => ambiguous.Normalize<Box2<Pair2<Point2>>>(new(1,
            BoxSchema(1, Inline(PairSchema(1, PointSchema(1)))), new Box1<Pair1<Point1>>(default)),
            BoxSchema(2, Inline(PairSchema(2, PointSchema(2))))));
        Assert.Empty(Calls);
    }

    [Fact]
    public void UInt32RepresentationDoesNotEraseNumericStringAndReferenceSlotSemantics() {
        Calls.Clear();
        ...
        foreach (DurableFieldInfo prior in slots) {
            foreach (DurableFieldInfo next in slots.Where(slot => slot != prior)) {
                ...
                context.AddRules(new(typeof(KeepRules), [], allowKeepExact: true));
                Assert.Throws<InvalidDataException>(() => context.Normalize<Box2<uint>>(
                    new(1, new("Semantic", 1, prior), new Box1<uint>(7)), new("Semantic", 2, next)));
            }
        }
        Assert.Empty(Calls);
        ...
        Assert.Equal(7u, explicitConversion.Normalize<Box2<uint>>(
            new(1, new("Semantic", 1, new DurableFieldInfo(1, TypeTag.String)), new Box1<uint>(7)),
    ...
    public void SameStateTypesCanSelectDifferentBusinessKeysAndInheritedDeclarationSegments() {
        ...
        Assert.Equal(new Two2(4, 40), context.Normalize<Two2>(new(3, source, new Two1(3, 4)), target));
        ...
        Assert.Throws<InvalidDataException>(() => wrong.Normalize<Box2<int>>(
            new(1, BoxSchema(1, new(1, TypeTag.Int32)), new Box1<int>(1)), BoxSchema(2, new(1, TypeTag.Int32))));
        Assert.Empty(Calls);
    }
    ...
    public void UndeclaredAndWrongTypedQueriesFailAtInvocationAndBusinessExceptionsDoNotRetry() {
        ...
        DurableSchema source = BoxSchema(1, new(1, TypeTag.Int32)), target = BoxSchema(2, new(1, TypeTag.Int32));
        Assert.Throws<InvalidOperationException>(() => unknown.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target));
        Assert.Single(Calls);
        Calls.Clear();
        ...
        Assert.Throws<InvalidOperationException>(() => wrongType.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target));
        Assert.Single(Calls);
        Calls.Clear();
        ...
        Assert.Throws<ArithmeticException>(() => throwing.Normalize<Box2<int>>(new(1, source, new Box1<int>(5)), target));
    ...
    public void SharedDependencyPlansCreateOneToolPerInvocationInsteadOfExpandingTheDag() {
        ...
        ObjectStateRecord row = new(51, source, Activator.CreateInstance(priorType)!);
        ...
        Calls.Clear();
        run(context, new(52, source, Activator.CreateInstance(priorType)!), target);
        Assert.All(Calls, item => Assert.Equal(52u, item.Context.ObjectId));
    }
    ...
    public void CachedPlansRecheckRegisteredInlineClosuresBeforeCallbacksAndFailedBindingsCanRetry() {
        ...
        Assert.Throws<InvalidDataException>(() => context.Normalize<Box2<Point2>>(new(1, source, new Box1<Point1>(new(2))), target));
        Assert.Empty(Calls);
        context.AddRules(new(typeof(CoordinateRules), [InlineRule("Point", 1, 2, nameof(ScalePoint))]));
        Assert.Equal(20, context.Normalize<Box2<Point2>>(new(1, source, new Box1<Point1>(new(2))), target).Value.X);
        Calls.Clear();
        context.Registered[(TypeExpr.Named("Point"), 1)] = new("Point", 1, SchemaKind.InlineValue, new DurableFieldInfo(1, TypeTag.Int64));
        Assert.Throws<InvalidDataException>(() => context.Normalize<Box2<Point2>>(new(2, source, new Box1<Point1>(new(2))), target));
        Assert.Empty(Calls);
    }
    ...
    private static TestContext SemanticContext(DurableFieldInfo prior, DurableFieldInfo next) {
        ...
        context.Definitions.Add("Semantic", new("Semantic", SchemaKind.ReferenceObject, 0, null, [
            new("Semantic", 1, SchemaKind.ReferenceObject, 0, [new(1, StateBindingContext.NominalType(prior))], stateTypeDefinition: typeof(Box1<uint>)),
            new("Semantic", 2, SchemaKind.ReferenceObject, 0, [new(1, StateBindingContext.NominalType(next))], stateTypeDefinition: typeof(Box2<uint>)),
    ...
    private static void CopyId(in uint prior, out uint next, UpgradeContext context) { Calls.Add(("copy", context)); next = prior; }
