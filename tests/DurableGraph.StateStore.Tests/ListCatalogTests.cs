...
public sealed class ListCatalogTests {
    ...
    public void NestedConstructorsAndInlineDependencyHaveIndependentGoldens() {
        ...
        byte[] golden = Convert.FromHexString("01010204010F0804080102");
        Assert.Equal(golden, SchemaCatalogWireCodec.Write([row], CatalogTestData.Empty));
        Assert.Equal(row.Layout, Assert.Single(SchemaCatalogWireCodec.Read(golden, CatalogTestData.Empty)).Layout);
        ...
        byte[] inlineGolden = Convert.FromHexString("010202020203500001000101020304011002");
    ...
    [InlineData("010102040202")] // Future codec.
    [InlineData("01010204810002")] // Noncanonical codec.
    [InlineData("010102040100")] // Unknown slot.
    [InlineData("010102040111")] // Open slot.
    [InlineData("01010204010F0104")] // String has a canonical dedicated slot.
    [InlineData("01010204011000")] // Null dependency.
    [InlineData("01010204011001")] // Builtin string dependency.
    [InlineData("01010204011002")] // Self dependency.
    [InlineData("01010204011003")] // Forward dependency.
    [InlineData("01010204010200")] // Extra byte.
    [InlineData("01020204010203040102")] // Duplicate layout at another ID.
    ...
    public void InlineDependencyRejectsClassAndContainerNodesAndKeepsSchemaDepthLimit() {
        byte[] dependency = Convert.FromHexString("01010304011002");
        ...
        byte[] golden = Convert.FromHexString("010182020401108102");
