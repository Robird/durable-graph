...
public sealed class NominalReferenceSchemaWireTests {
    ...
    public void ChangingOnlyNominalFamilyConflictsWithPersistedDefinition() {
        DurableSchema first = new("A", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "B"));
        DurableSchema changed = new("A", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "C"));
    ...
    public void RuntimeOperandIsExclusiveAndRequiredAndSelfReferenceIsNotAnExactDependency() {
        Assert.ThrowsAny<ArgumentException>(() => new DurableFieldInfo(1, TypeTag.DurableReference));
        Assert.ThrowsAny<ArgumentException>(() => new DurableFieldInfo(1, TypeTag.DurableReference, " "));
        Assert.ThrowsAny<ArgumentException>(() => new DurableFieldInfo(1, TypeTag.UInt32, "A"));
        DurableSchema self = new("A", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "A"));
    ...
    public void SchemaStoreRegistersNominalCyclesWithoutTargetClosureAndColdConflictIsAtomic() {
        ...
        DurableSchema a = new("A", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "B"));
        DurableSchema b = new("B", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "A"));
        try {
            ...
            using (IRbfFile file = RbfFile.OpenExisting(path)) {
                ...
                Assert.Throws<SchemaConflictException>(() => store.RegisterBatch([
                    new("Independent", 1), new("A", 1, new DurableFieldInfo(1, TypeTag.DurableReference, "C"))]));
