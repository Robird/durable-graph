...
public sealed class RepresentationIntegrationTests : IDisposable {
    ...
    [Fact]
    public void LegacyAndIdBasesCoexistAcrossColdReopenAndDeltaInheritsItsTerminatingBase() {
        ...
        using (SegmentStore segments = OpenState()) {
            ...
            // Independent v3 durable header golden for W v1, followed by StringId 2 and Value 7.
            // New producers cannot emit this historical format.
            byte[] legacy = Convert.FromHexString("030202035700010207");
            ...
            first = states.AppendDurably(StateRevision.CreateObjectHeadMapBase(null,
                [ObjectVersionRecord.CreateBase(1, legacy), ObjectVersionRecord.CreateBase(2, text.Body)], []));
        ...
        using (SegmentStore segments = SegmentStore.OpenReadOnlyExisting(Path.Combine(_root, "state"), Options)) {
            ...
            Assert.Null(BaseObjectBodyCodec.Decode(oldChain.Records[0].Record.Body, schemas).RepresentationId);
            ...
            Assert.Equal(schemaTail, schemaFile.TailOffset); // Legacy reads do not invent registrations.
        }
