...
2026-09-07 用户明确裁剪以下范围；这是支持合同，不表示相关 codec 或加载流程已经实现：

- 数组仅支持零下界的 SZ VectorArray `T[]` 和有限 rank 的多维数组。任何维度非零下界，
  以及非 SZ 的 rank-1 数组 `T[*]`（即使下界为零），均明确拒绝，不转换成其他形状。
  多维数组 rank 上界取 3 或 4，由数组分片结合 TypeCode 与生成循环的实现选定；超出所选上界拒绝。
  可用 VectorArray、Rank-2、Rank-3、可选 Rank-4 的独立 tag 省去额外 rank 参数；
  具体 wire 值未定，元素类型及每维长度仍须表达。可静态识别的不支持类型由 SG 拒绝，
  其余在 Capture/读取边界校验，不静默降级。此项不同时裁决所有元素类型或泛型组合的支持范围。
...
3. 为领域对象分配实例，建立 ObjectId 到实例的映射；具体构造方式另行裁决。
4. 填充持久字段并连接对象引用，利用先分配的完整映射保留共享和循环。
5. 完成框架的持久数据/引用校验后交付 World，不能暴露解码、升级或引用连接的半成品。

交付后的 Transient 重建由用户代码负责，属于宿主阶段。MVP 不提供自动 Transient hook，
