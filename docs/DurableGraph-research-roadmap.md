...
[DB-059 跨程序集模型目录与类型组合](design-branches/0059-cross-assembly-model-composition-slice.md) 已贯通
external nominal / 动态表示参数、显式 Family 导出及普通 Model/reader 的 nominal 检查；能力与证据从该片进入。
后续跨程序集工作限于固定外部 inline/base 模板、外部规则发现等剩余边界，不再把整个类型组合路径列为待实现。
下一片推荐 [DB-060 固定外部 inline 与只读历史依赖](design-branches/0060-cross-assembly-inline-history-slice.md)（Proposed，尚未实施）：
补齐直接外部值字段，按需导入完整模板并复用定义库已有公开生成能力；owner 显式升版，history 分库维护。
外部 base 与自动业务规则发现继续独立，不将本片扩大为全部 inline DTO 动态参数化或新 State 格式。
...
| 跨程序集与一般类型形状 | 固定外部 inline 的只读材料、history 所有权及 helper 合同已有 DB-060 提案，尚未实施；跨库 base 的声明层访问另片。跨程序集业务规则扫描与独立闭合历史账本分别裁决，不与 nominal 组合混同。DateTime 的 Local/DST 合同、native int 等各自按需求选择；boxed value identity 已排除 MVP |
