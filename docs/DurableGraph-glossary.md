...
| <a id="object-layout"></a>**精确对象布局（ObjectLayout / ArrayLayout）** | ObjectLayout 是 String / 用户 exact DurableSchema / exact ArrayLayout 的受控判别值。ArrayLayout 保存 codec 版本、数组构造和完整元素槽；元素 inline Schema 精确绑定，引用元素只保存目标名义约束。shape 不属于类型布局。 | [ObjectLayout / ArrayLayout](../src/DurableGraph/ObjectLayout.cs)、[Base 类型头](../src/DurableGraph.StateStore/BaseObjectBodyCodec.cs) |
