...
| <a id="field-id"></a>**持久字段编号（FieldId）与声明段** | FieldId 在声明它的 Schema 内唯一，继承链各段独立编号；基类和派生类都可有字段 1。Schema 字段类型由 `TypeTag` 描述；引用字段的类型 tag 与 DTO 中存 ID 的 `UInt32` 槽类型不同。 | [DurableFieldInfo](../src/DurableGraph/DurableFieldInfo.cs)、[字段展开发出逻辑](../src/DurableGraph.Generator/DurableSchemaGenerator.BinaryBody.cs) |
| <a id="state-dto"></a>**版本化状态 DTO（Versioned state DTO）**／SG 生成 `__DurableBinaryBody.Vn` | 与一版 exact Schema 配对的纯状态值，包含祖先字段。当前生成 readonly/unmanaged struct：标量直接存值，string/durable 引用槽存 `uint` ID。DTO 是内存值，不是领域实例或编码 bytes。 | [DTO 发出逻辑](../src/DurableGraph.Generator/DurableSchemaGenerator.BinaryBody.cs)、[引用 DTO 示例](../tests/DurableGraph.Tests/GeneratedReferenceBodyTests.cs) |
| <a id="schema-history"></a>**Schema 历史材料（Schema history）**／`.dgsnapshot` | 构建期积累的类型形状记录，供 SG 重建旧版 Schema、DTO 和代码；不是某次游戏状态。history manifest 描述本次编译的候选定义，Publish/Verify 与已接受历史比对。 | [SnapshotHistoryTool](../src/DurableGraph.Build/SnapshotHistoryTool.cs)、[历史解析与 manifest 生成](../src/DurableGraph.Generator/DurableSchemaGenerator.cs) |
...
| <a id="object-state-record"></a>**对象状态记录**／`CapturedObject` | 不可变单对象内存记录：`ID + kind + exact Schema/DTO`，或 `ID + string 内容`。同一代码类型同时承载捕获、解码与归一化结果；仅有该行不证明它是已接受候选。 | [CapturedObject](../src/DurableGraph/CapturedObject.cs) |
...
BaseObjectPayloadCodec  → [Base 类型头 | 原始 Base body]
...
| 项目内术语／代码符号 | 释义与示意 | 关键代码 |
|---|---|---|
| <a id="prepared-content"></a>**已准备内容（PreparedBase / PreparedDelta）** | 拥有独立 bytes、决策后可复用的容器；Delta 还保存 `HasChanges`。无变化仍可含非空零位图。容器不认证 Schema/prior；`PreparedBase` 在不同调用处可以装原始 body 或带类型头的 Base body。 | [PreparedBase](../src/DurableGraph.StateStore.Serialization/Serialization/PreparedBase.cs)、[PreparedDelta](../src/DurableGraph.StateStore.Serialization/Serialization/PreparedDelta.cs) |
| <a id="body-payload"></a>**原始对象 body、Base 类型头、ObjectVersion payload** | body 是 DTO/string 的内容编码；Base 类型头区分 string 与 durable exact SchemaKey；ObjectVersion payload 再含 kind、可选 prior、长度及 body。讨论字节数时必须指出这一层，不能仅凭属性名 `Payload` 判断。 | [BaseObjectPayloadCodec](../src/DurableGraph.StateStore/BaseObjectPayloadCodec.cs)、[ObjectVersionPayloadSize](../src/DurableGraph.StateStore.Storage/ObjectVersionPayloadSize.cs) |
...
| `Snapshot` | `.dgsnapshot` 指[Schema 历史材料](#schema-history)；legacy SG `__DurableSnapshotVn` 是另一条早期状态表示路径；当前产品用[版本化 DTO](#state-dto)及明确的 stored/current/candidate 视图。三者不能统一替换成一个词。 | [SG legacy/history 分支](../src/DurableGraph.Generator/DurableSchemaGenerator.cs)、[早期 InMemoryStateStore](../src/DurableGraph/InMemoryStateStore.cs) |
| `SchemaOnly` | 当前配置选择 SchemaOnly 路径；同时启用 `GenerateBinaryBody` 时仍生成 DTO/body。不要按字面解释成“绝不生成序列化代码”。 | [GenerateBinaryBodies](../src/DurableGraph.Generator/DurableSchemaGenerator.BinaryBody.cs) |
...
| Base Revision、`StateRevision.CreateBase/CreateDelta` | 这两个工厂选择[目录 Base/Delta](#head-map)，不保证本地对象都是相同表示。`BaseSchema` 另属继承关系。 | [StateRevision](../src/DurableGraph.StateStore.Storage/StateRevision.cs) |
| `CapturedObject`、`PreparedBase.Payload`、早期 `EstimatedBaseWriteBytes` | 前两者仍有阶段歧义：CapturedObject 也装解码结果，PreparedBase 不区分 raw/wrapped。内部策略输入已改用 `BasePayloadBytes`、`DeltaPayloadBytesUpperBound`、`ReconstructionPayloadBytes`，明确 B 是精确值、D 是上界、H 是已有重建链 payload。公开符号迁移仍需单独核对消费者影响。 | [对象状态记录](#object-state-record)、[已准备内容](#prepared-content)、[ObjectSaveEstimate](../src/DurableGraph.StateStore/ObjectSaveEstimate.cs) |
| head、Parent、previous | head 指明“对象 head”或“发布 head”；Parent 指明 Revision；内存 Previous 不能自动当作已认证 Parent。`revisionHead` 一类参数名也不证明该地址已发布。 | [Parent/prior](#parent-prior)、[提交发布](#prepare-append-commit) |
