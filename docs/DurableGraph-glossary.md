...
| <a id="dictionary-key-equality"></a>**字典查找相等与持久键相等** | 查找相等由领域 Dictionary 的 comparer 决定；持久相等以同 exact key 槽的完整 canonical Base bytes 对应 frozen 条目。业务可忽略需要保存的 Timestamp；内容相同但 ObjectId 不同的 string 键也可查找等价而持久状态不同，后者应 Remove+Add。DB-054 已分开两种比较；DB-055 新方向允许当前业务规则并分开历史 DTO 验证与当前领域构造，具体新增模式尚未实现。 | [Dictionary body](../src/DurableGraph/DictionaryStateReader.cs)、[key policy](../src/DurableGraph/DictionaryKeyPolicy.cs)、[复合 Key 设计](design-branches/0055-composite-dictionary-key-design.md) |
