...
| `CapturedObject`、`PreparedBase.Payload`、`EstimatedBaseWriteBytes` | 名称有阶段歧义：CapturedObject 也装解码结果；PreparedBase 不区分 raw/wrapped；现产品提供给策略的 Base 字节数已是精确值，Delta 仍有地址上界。解释时使用上文限定词，后续符号迁移另作影响核对。 | [对象状态记录](#object-state-record)、[已准备内容](#prepared-content)、[ObjectSaveEstimate](../src/DurableGraph.StateStore/ObjectSaveEstimate.cs) |
