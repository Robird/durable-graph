...
| <a id="schema-history"></a>**Schema 历史材料（Schema history）**／`.dgschema` | 构建期积累的定义模板记录，含声明 arity、参数模式与固定依赖版本，供 SG 重建历史状态代码；不是某次对象状态，也不枚举全部闭合布局。当前新写 v6：v5 引入 List 模式 `l(element)`，v6 引入 Nullable 模式 `q(child)`；已有 v1–v5 文件原样保留，各版拒绝不属于该版的构造。manifest 描述本次候选；已接受历史须独立闭包，候选不能修补旧缺口。 | [SchemaHistoryTool](../src/DurableGraph.Build/SchemaHistoryTool.cs)、[模板解析](../src/DurableGraph.Generator/DurableSchemaGenerator.TemplateHistory.cs) |
...
| <a id="dictionary-key-equality"></a>**字典查找相等与持久键相等（设计中）** | 查找相等由 Dictionary 的比较策略决定唯一键；DB-054 拟以同 exact key 槽的 canonical Base bytes 对应 frozen 条目。内容相同但 ObjectId 不同的 string 键可查找等价而持久状态不同；后者应 Remove+Add。不是 SchemaKey，也不改变现有 StateEquals 的定义。 | [DB-054 推荐设计](design-branches/0054-dictionary-content-object-slice.md)；尚无产品 Dictionary 实现 |
