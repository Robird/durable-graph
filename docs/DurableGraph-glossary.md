...
| <a id="dictionary-key-equality"></a>**字典查找相等与持久键相等** | 查找相等由 Dictionary 的比较策略决定唯一键；DB-054 以同 exact key 槽的 canonical Base bytes 对应 frozen 条目。内容相同但 ObjectId 不同的 string 键可查找等价而持久状态不同；后者应 Remove+Add。不是 SchemaKey，也不改变现有 StateEquals 的定义。 | [Dictionary body](../src/DurableGraph/DictionaryStateReader.cs)、[key policy](../src/DurableGraph/DictionaryKeyPolicy.cs) |
