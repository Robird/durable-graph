...
> 状态：Proposed — 主代理与独立设计审查者的下一分片推荐，等待用户裁决；尚未实施。
> 日期：2026-09-06。规划时 HEAD 为 `0e5608d`，产品内容证据基线为 `54a33df`。
> 本文是规划，不授权产品实现，也不冻结最终对象持久格式。
...
`ApplyDelta(prior, WriteDelta(prior, current))` 的完整 Base bytes 与 `Write(current)` 完全一致；
相等比较也与完整 Base bytes 的相等一致。错误输入失败，prior 与已封存候选不被修改。
...
不增加新的 opt-in 开关、Delta DTO、缓存的变更计划、程序集或通用 codec 接口。
方法命名在实施中可微调，形状推荐如下：

```csharp
internal static bool StateEquals(in V1 left, in V1 right);
internal static void WriteDelta(
    ref BinaryPayloadWriter writer, in V1 prior, in V1 current);
...
推荐以当前支持槽位的完整 Base 编码是否相同为语义判据，生成代码直接逐槽比较，不实际编码后再比较。
...
浮点按位是本片的新推荐决策，尚待用户采纳。若选择数值相等，需要另外决定被忽略位变化后应保留哪份内容，
会改变上面的精确重建/相等判据。当前推荐保留已经能够无损编码的信息。

### 3.3 Delta body：固定字段位图与新值

对于 exact Vn 中的 N 个槽位，建议编码：
...
writer 必须只置位真正变化的槽；reader 建议同时拒绝“置位但新值与 prior 持久相等”的冗余变更，
...
上层依据 `StateEquals` 分类为 NoChange，不因取得了 Delta bytes 就生成 Update。
...
失败不修改 prior，不返回半成品。reader 允许已前进，writer 失败不承诺回滚，沿用现有 body 接缝。
...
测试/首个显式调用者用 `ArrayBufferWriter<byte>` 调现有 Write 与新 WriteDelta，读取实际 WrittenCount。
这是先得到正确内容和实际长度的直接方法，接受先编码成本；未来 Save 是否缓存编码或独立估算留给真实消费者。
...
| 验收组 | 必须观察到的结果 |
|---|---|
| 相等与重建律 | 各支持槽位，equal/single/all change；Apply 的 Base bytes 精确等于 current；重复比较/写入稳定且不改 DTO |
...
| 包交付 | 单 PackageReference 的既有 consumer 增加 comparer/Delta 调用，确保生成代码只依赖可访问 API |

建议按依赖组织实施：

1. 主代理确认本文比较和格式决策，固定方法形状、错误边界及黄金样例。
2. 生成器子任务：集中扩展 BinaryBody 文件，比较表达式由一个生成 helper 复用；保持静态调用。
...
产品预期改动集中于 Generator，tests 与包消费者可分工；不预先扩大 Runtime/Storage API。
测试条件应包含实际 generated-code 执行，不能只用生成文本断言证明编码正确。
本次仅规划，因此未运行产品 build/tests；实施后应记录实际命令和结果。
...
DTO 升级、领域 Restore、一般引用递归、struct/数组/BCL 扩展、ID 回收、性能优化或新增策略。
