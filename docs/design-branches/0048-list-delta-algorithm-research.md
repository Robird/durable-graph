...
| 高 | Apply 成本、固定链与真实策略形成的冷读链成本 | 不要求本片改策略、Frame cache 或采用原地链 accumulator |
...
必须测两组：固定相同链深度；以及相同编辑序列、相同 X/Y 下，让实际策略自由选择产生的链。
记录 Base 次数、链深、累计 payload、分配、CPU 和物理读量。这是本轮选型必须观察的风险，
不据此直接加入最大链深参数或更换策略。若实测不可接受，单列恢复器/策略议题后裁决。
...
首个实验可用 canonical bytes 建立正确性和成本基准，再判断是否值得增加静态比较能力。
引用元素比较 canonical ObjectId；浮点保留 bit pattern；struct 只看持久字段。
...
先做局部可执行实验，再修成施工合同；不同时建设大型 benchmark 平台、tracking 容器或新策略。

1. **语义与比较成本**：两类范围语义的 roundtrip、畸形输入；canonical 元素匹配成本；纯 Copy 与稀疏 Patch 示例字节。
2. **候选对照**：DB-047/完整 Base/前后缀单间隙基线/A/B；只有大块重排或其它用例显示有意义差距时，增加 C。
   对 same grammar 比 matcher，对固定 matcher 比 grammar，记录回退率和总代价。
3. **持久集成试验**：固定链与实际 X/Y 形成的链；判断最终 matcher、预算、比较接口及 opcode，随后才实施产品。
...
链实验报告 actual policy 路径和固定深度路径，不以一者代替另一者。

最终裁决应回答：最小候选是否已取得主要插删收益，强候选多花的复杂度是否有实测回报，
稀疏 struct 是否退化，比较材料是否成为主成本，以及更长 Delta 链是否仍可接受。
