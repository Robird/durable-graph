...
$consumerProject = Join-Path $PSScriptRoot "TemporalScalarConsumer/TemporalScalarConsumer.csproj"
$runId = "temporal-scalar-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss'))-$PID-$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
try {
    if ([string]::IsNullOrWhiteSpace($PackageSource)) {
        ...
        $Version = "0.0.0-temporal-scalar-e2e.$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss')).$PID"
        foreach ($project in @(
    ...
    $assembly = Join-Path $output "Debug/net10.0/Atelia.TemporalScalarConsumer.dll"
    $hashes = @{}
    $contents = @{}
    foreach ($stage in @(
        @{ Number = 1; Count = 3; Expected = "TemporalScalarSeed:True:ThreeScalarCompositions:True:ExactOffset:True:OffsetKeyRemoveAdd:True:ColdReopen:True" },
        @{ Number = 2; Count = 5; Expected = "TemporalScalarUpgrade:True:DeletedInlineClr:True:ExplicitOwnerAndValueUpgrade:True:HistoricalExact:True:ForcedBaseThenDelta:True:ColdReopen:True" }
    )) {
        ...
        if ($LASTEXITCODE -ne 0 -or $actual -ne $stage.Expected) { throw "TemporalScalar stage $($stage.Number) failed; output was '$actual'." }
        Write-Host $actual
    }
    Write-Host "TemporalScalar package consumer probe passed. Artifacts: $workRoot"
}
