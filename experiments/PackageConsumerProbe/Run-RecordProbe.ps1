...
$consumerProject = Join-Path $PSScriptRoot "RecordConsumer/RecordConsumer.csproj"
$runId = "record-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss'))-$PID-$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
try {
    if ([string]::IsNullOrWhiteSpace($PackageSource)) {
        ...
        $Version = "0.0.0-record-e2e.$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss')).$PID"
        foreach ($project in @(
    ...
    $assembly = Join-Path $output "Debug/net10.0/Atelia.RecordConsumer.dll"
    $hashes = @{}
    foreach ($stage in @(
        @{ Number = 1; Count = 4; Expected = "RecordSeed:True:ReadonlyPositionalGeneric:True:PersistentKeyReplacement:True:StableModes:True" },
        @{ Number = 2; Count = 7; Expected = "RecordUpgrade:True:DeletedGenericAndNestedTypes:True:ExplicitDoubleSlotUpgrade:True:ForcedBaseThenDelta:True:ColdReopen:True" }
    )) {
        ...
        if ($LASTEXITCODE -ne 0 -or $actual -ne $stage.Expected) { throw "Record stage $($stage.Number) failed; output was '$actual'." }
        Write-Host $actual
    }
    Write-Host "Record package consumer probe passed. Artifacts: $workRoot"
}
