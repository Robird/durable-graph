...
$consumerProject = Join-Path $PSScriptRoot "EnumConsumer/EnumConsumer.csproj"
$runId = "enum-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss'))-$PID-$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
try {
    if ([string]::IsNullOrWhiteSpace($PackageSource)) {
        ...
        $Version = "0.0.0-enum-e2e.$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss')).$PID"
        foreach ($project in @(
    ...
    $assembly = Join-Path $output "Debug/net10.0/Atelia.EnumConsumer.dll"
    $hashes = @{}
    foreach ($stage in @(
        @{ Number = 1; Count = 4; Expected = "EnumSeed:True:UnknownBits:True:GenericNullableAndArrays:True:SharedListDelta:True:FrozenPreparation:True" },
        @{ Number = 2; Count = 8; Expected = "EnumUpgrade:True:HistoricalExact:True:DeletedDomainEnum:True:ExplicitLiftAndOwners:True:ForcedBaseThenDelta:True:ColdReopen:True" }
    )) {
        ...
        if ($LASTEXITCODE -ne 0 -or $actual -ne $stage.Expected) { throw "Enum stage $($stage.Number) failed; output was '$actual'." }
        Write-Host $actual
    }
    Write-Host "Enum package consumer probe passed. Artifacts: $workRoot"
}
