...
try {
    ...
    foreach ($stage in $stages) {
        ...
        foreach ($file in $historyFiles) {
            ...
            if (-not $oldHistoryHashes.ContainsKey($file.Name) -and -not $content.StartsWith("// durable-graph-schema-history:7`n", [StringComparison]::Ordinal)) {
                throw "Newly published Schema history is not canonical format v7."
            }
