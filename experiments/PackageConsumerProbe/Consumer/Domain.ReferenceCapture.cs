...
internal static class ReferenceCaptureExercise {
    internal static void Run() {
        ...
        using (var capture = session.BeginCapture()) {
            Require(CaptureHero.__DurableState.AddRoot(capture, hero) == 1, "First root ID.");
            Require(CaptureItem.__DurableState.AddRoot(capture, item) == 2, "Second root ID.");
            ...
            Require(ReferenceEquals(first, session.Current) &&
                first.Objects.Single(entry => entry.Id == 1).GetState<CaptureHero.__DurableState.V1>().Segment1Field2 == 7,
        ...
        uint retriedId;
        using (var capture = session.BeginCapture()) {
            Require(CaptureHero.__DurableState.AddRoot(capture, hero) == 1, "Live root ID remains stable.");
            Require(CaptureItem.__DurableState.AddRoot(capture, item) == 2, "Other live root remains stable.");
            var candidate = capture.Seal();
            var state = candidate.Objects.Single(entry => entry.Id == 1).GetState<CaptureHero.__DurableState.V1>();
        ...
        using (var capture = session.BeginCapture()) {
            Require(CaptureHero.__DurableState.AddRoot(capture, hero) > retriedId,
