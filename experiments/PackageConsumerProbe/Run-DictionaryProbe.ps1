...
$consumerProject = Join-Path $PSScriptRoot "DictionaryConsumer/DictionaryConsumer.csproj"
$runId = "dictionary-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss'))-$PID-$([Guid]::NewGuid().ToString('N').Substring(0, 8))"
...
try {
    if ([string]::IsNullOrWhiteSpace($PackageSource)) {
        ...
        $Version = "0.0.0-dictionary-e2e.$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss')).$PID"
        foreach ($project in @(
    ...
    $assembly = Join-Path $output "Debug/net10.0/Atelia.DictionaryConsumer.dll"
    $hashes = @{}
    foreach ($stage in @(
        @{ Number = 1; Count = 5; Expected = "DictionarySeed:True:SharedComparersAndCycles:True:KeyAddressedDelta:True:UnorderedNoChange:True:FrozenPreparation:True" },
        @{ Number = 2; Count = 7; Expected = "DictionaryUpgrade:True:HistoricalExact:True:DeletedKeyAndValueTypes:True:IndependentKeyValueRules:True:SharedOwnerOnce:True:ForcedBaseThenDelta:True:ColdReopen:True" }
    )) {
        ...
        foreach ($file in $files) {
            ...
            if (-not $hashes.ContainsKey($file.Name) -and -not $text.StartsWith("// durable-graph-schema-history:7`n", [StringComparison]::Ordinal)) { throw "Expected canonical v7 history." }
        ...
        if ($LASTEXITCODE -ne 0 -or $actual -ne $stage.Expected) { throw "Dictionary stage $($stage.Number) failed; output was '$actual'." }
        Write-Host $actual
    }
    Write-Host "Dictionary package consumer probe passed. Artifacts: $workRoot"
}
