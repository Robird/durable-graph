...
internal sealed record Settings(int[] Counts, int Repeats, int Rounds, int DiffRepeats, int Seed,
    int ReadAmplification, int BaseBudgetPercent, string Output, string Baseline) {
    internal static Settings Parse(string[] args) {
        ...
        int[] counts = Get("--counts", "32").Split(',').Select(int.Parse).Distinct().Order().ToArray();
        ...
        if (values.Count != 0 || counts.Length == 0 || counts.Any(count => count is < 8 or > 50_000) ||
        ...
        return new(counts, repeats, rounds, diffRepeats, seed, x, y, output, baseline);
    }
}
