...
internal static class ObjectVersionDictionaryReader {
    ...
    private static AbsoluteFrameAddress ResolveEarlierAddress(
        ...
        try {
            ...
            throw new InvalidDataException(
                $"{role} in Revision {containingRevisionAddress} has an invalid relative ticket for object {objectId}.",
                exception);
        }
        ...
        if (resolvedAddress.FileNumber == containingRevisionAddress.FileNumber &&
            ...
            throw new InvalidDataException(
                $"{role} in Revision {containingRevisionAddress} points to non-earlier frame {resolvedAddress} for object {objectId}.");
        }
    ...
    private static Frame ReadFrame(
        ...
        try {
            ...
            throw new InvalidDataException(
                $"{role} {address} is missing while looking up object {objectId}.",
                exception);
        }
    }
}
