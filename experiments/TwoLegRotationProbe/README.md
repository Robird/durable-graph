...
| Adaptive read-amplification/Base-budget v0 | Two payload-proxy parameters now select Base/Delta, reserve one old-A progress action per Stay, and trigger Rotate below a strict debt share. Exact threshold/apply and typed capacity rejection are closed; in the first matched cadence paced locally dominates adaptive W/P/F while final R misses adaptive's intermediate hot-chain reset. | Payload heuristic and one synthetic counterexample only—not encoded/physical byte authority, a fifth score, tuned defaults, or a general winner. [`READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md`](READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md), [`ReadAmplificationBaseBudgetPolicyIntegrationTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyIntegrationTests.cs), [`ReadAmplificationBaseBudgetPolicyCapacityTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyCapacityTests.cs) |
