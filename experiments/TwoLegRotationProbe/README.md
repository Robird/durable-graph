...
| Equal-byte migration membership | With equal exact Stay layouts, choosing the singleton-frame object releases one A Frame immediately while smallest-ObjectId leaves both A Frames live. | A test-local source-topology witness, not temperature inference, a weighted winner, or a default policy. [`RotationPolicyComparisonTests.cs`](Tests/RotationPolicyComparisonTests.cs) |
