...
Corpus revision 2 contains two matched no-migration/paced treatment pairs. The
fixed-seed mixed pair is an exact negative control because neither evaluated step has
an eligible A-debt `NoChange`; both sides produce the same raw outcome. In the
handwritten pair, paced migration causes a workload rotation before the unconditional
terminal rotation, so the final scopes and W/P/F/R differ. That is executable evidence
of closed-horizon coupling, not a general policy ranking. Exact vectors and pair
identity guards live in [`BenchmarkV1RunnerTests.cs`](Tests/BenchmarkV1RunnerTests.cs).
...
| Controlled migration pacing | Moving one A-debt object on each fixed-target Stay can smooth Rotate writes, but those B records become next-epoch Previous debt. A fixed-cadence two-epoch witness makes paced re-migrate the same three cold objects and exposes a `W/P/F/R` Pareto trade rather than a free peak reduction. | The cadence is an experimental control, not a product trigger or steady-state proof; ObjectId is deterministic assignment, not coldness. [`RotationPolicyComparisonTests.cs`](Tests/RotationPolicyComparisonTests.cs), [`FixedCadenceTerminalLiabilityTests.cs`](Tests/FixedCadenceTerminalLiabilityTests.cs) |
| Adaptive read-amplification/Base-budget v0 | Two payload-proxy parameters select Base/Delta, reserve one old-A progress action per Stay, and trigger Rotate below a strict debt share. Realized accepted-head H/B is now observable without entering canonical W/P/F/R. `(3,5%)` and `(4,4%)` are identical when soft-budget object indivisibility plus the progress floor mask their differences; an isolated threshold-band fixture instead observes 4B less W for 260B more final R. | Payload heuristic and bounded synthetic evidence only—not encoded/physical byte authority, a fifth score, Peak improvement, tuned defaults, or a general winner. [`READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md`](READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md), [`ReadAmplificationBaseBudgetPolicyIntegrationTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyIntegrationTests.cs), [`ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs), [`RealizedReconstructionPayloadAmplificationDiagnosticTests.cs`](Tests/RealizedReconstructionPayloadAmplificationDiagnosticTests.cs), [`ReadAmplificationBaseBudgetPolicyCapacityTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyCapacityTests.cs) |
...
| Benchmark-v1 matched consumer | Two no-migration/paced pairs share exact fixture, trace, target, and evaluator protocols. The mixed negative control is identical; the active handwritten pair exposes horizon coupling. A named continuation then holds final scope advances equal and retains distinct raw vectors/debt layouts. | Neither one-settlement nor equal-scope evidence selects a general winner; single-A-Frame bootstrap, writer-only JSON, no score/parser/CLI. [`EVALUATOR-V1.md`](EVALUATOR-V1.md), [`BenchmarkV1RunnerTests.cs`](Tests/BenchmarkV1RunnerTests.cs), [`FixedTwoScopeAdvanceHorizonTests.cs`](Tests/FixedTwoScopeAdvanceHorizonTests.cs), [`BenchmarkV1JsonTests.cs`](Tests/BenchmarkV1JsonTests.cs) |
