...
admission, apply, settlement, final-state validation, and W/P/F/R/L/T remain Arena-owned.
...
than a target/decision cross-product. Corpus revision 12 runs four strategies
...
| Active hundred mixed | The fixed-seed generator bootstraps 100 persistent objects with equal Field/List weights, then updates exactly 60 distinct objects per round for 64 rounds. This combines a large migration backlog, a changing NoChange pool, and sustained Delta opportunities. | Revision 12 shows Adaptive `(3,5%)` strictly dominating `(4,4%)` on W/P/F/R; the latter's smaller T was a terminal-phase artifact. The workload remains adjustable rather than golden. [`BenchmarkV1Corpus.cs`](Benchmarking/BenchmarkV1Corpus.cs), [`EVALUATOR-V1.md`](EVALUATOR-V1.md) |
