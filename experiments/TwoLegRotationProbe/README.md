...
| Controlled migration pacing | Moving one A-debt object on each fixed-target Stay can smooth Rotate writes, but those B records become next-epoch Previous debt. A common paced third epoch starts from aligned semantic/scope/debt/`H/B`/Current-tail state: its writes and final W/P/F/R converge while intermediate cold-head snapshots retain the shared-vs-split Frame history. | The cadence is an experimental control, not a product trigger or steady-state proof; checkpoint cold reads are neither actual nor cumulative IO, and ObjectId is deterministic assignment rather than coldness. [`RotationPolicyComparisonTests.cs`](Tests/RotationPolicyComparisonTests.cs), [`FixedCadenceTerminalLiabilityTests.cs`](Tests/FixedCadenceTerminalLiabilityTests.cs), [`EVALUATOR-V1.md`](EVALUATOR-V1.md) |
| Adaptive read-amplification/Base-budget v0 | Two payload-proxy parameters select Base/Delta, reserve one old-A progress action per Stay, and trigger Rotate below a strict debt share. Matched, read-threshold, and Base-fraction lower-bound witnesses now distinguish parameter masking, a local W/R trade, and a same-horizon target-timing trade; typed capacity rejection remains non-scoring. | Payload heuristic and bounded synthetic evidence only—not encoded/physical byte authority, a fifth score, tuned defaults, steady state, or a general winner. [`READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md`](READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md), [`ReadAmplificationBaseBudgetPolicyIntegrationTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyIntegrationTests.cs), [`ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs), [`ReadAmplificationBaseBudgetPolicyTargetBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyTargetBandTests.cs), [`ReadAmplificationBaseBudgetPolicyCapacityTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyCapacityTests.cs) |
