...
| Migration membership and byte pressure | At equal Stay cost, singleton-frame membership can release a whole A Frame; with payload skew, migrating small writes less while migrating large retires more Previous full-Frame closure bytes. | Test-local topology/size witnesses only—not physical reclamation, actual IO, temperature inference, a weighted winner, or a default policy. [`RotationPolicyComparisonTests.cs`](Tests/RotationPolicyComparisonTests.cs) |
| Preparatory capacity | Three large A Bases need two one-object B migration batches before a C evacuation fits. | One canonical witness does not prove search completeness. [`PreparatoryBaseMigrationTests.cs`](Tests/PreparatoryBaseMigrationTests.cs), [`CompletionCertificateTests.cs`](Tests/CompletionCertificateTests.cs) |
