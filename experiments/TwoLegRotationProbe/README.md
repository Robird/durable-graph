...
| Benchmark-v1 matched consumer | Two no-migration/paced pairs share exact fixture, trace, target, and evaluator protocols. The mixed negative control is byte-for-byte equivalent at the raw outcome level; the active handwritten pair exposes an extra workload rotation plus terminal-settlement coupling. | Four admitted raw vectors do not select a winner; single-A-Frame bootstrap, writer-only JSON, no score/parser/CLI. [`EVALUATOR-V1.md`](EVALUATOR-V1.md), [`BenchmarkV1RunnerTests.cs`](Tests/BenchmarkV1RunnerTests.cs), [`BenchmarkV1JsonTests.cs`](Tests/BenchmarkV1JsonTests.cs) |
