...
than a target/decision cross-product. Corpus revision 5 runs no-migration, paced, Adaptive
`(3,5%)`, and Adaptive `(4,4%)` over each of four frozen workloads. The hot-chain plus
Frame-unpin workload separates the read limits; the ordinary debt-share-dilution workload
then isolates the Adaptive pair's strict 4%/5% Rotate-or-Stay boundary. These are bounded
matched results, not tuned defaults, steady-state evidence, or a general policy ranking.
...
| Adaptive read-amplification/Base-budget v0 | Two payload-proxy parameters select Base/Delta, reserve one old-A progress action per Stay, and trigger Rotate below a strict debt share. Corpus revision 5 contains both an integrated hot-chain/Frame-unpin discriminator and an ordinary `E/G` debt-share boundary. At the latter, only the Adaptive pair shares the intended pre-boundary physical source; `(3,5%)` selects `Stay/Rotate/Stay`, while `(4,4%)` selects `Stay/Stay/Rotate`. | Payload heuristic and bounded synthetic evidence only—not encoded/physical byte authority, a fifth score, tuned defaults, steady state, or a general winner. Full-rewrite `Base==Delta` makes read limit and representation choice inert at the boundary. [`READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md`](READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md), [`BenchmarkV1ThresholdBandWorkloadTests.cs`](Tests/BenchmarkV1ThresholdBandWorkloadTests.cs), [`BenchmarkV1DebtShareDilutionWorkloadTests.cs`](Tests/BenchmarkV1DebtShareDilutionWorkloadTests.cs), [`ReadAmplificationBaseBudgetPolicyTargetBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyTargetBandTests.cs), [`ReadAmplificationBaseBudgetPolicyCapacityTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyCapacityTests.cs) |
...
| Benchmark-v1 matched consumer | Four strategies from the independent Baselines assembly share each frozen fixture/trace/horizon through the Arena whole-run contract. Revision 5 has 16 admitted cases over four workloads; earlier W/P/F/R vectors and trace hashes remain exact while the expanded manifest/report receive new canonical hashes. | This is an internal-track certified-product proof over four bounded workloads, not a general untrusted-artifact judge or full frontier; single-A-Frame bootstrap, writer-only JSON, no scalar score/parser/CLI. [`EVALUATOR-V1.md`](EVALUATOR-V1.md), [`StrategyArenaContractTests.cs`](Tests/StrategyArenaContractTests.cs), [`BenchmarkV1RunnerTests.cs`](Tests/BenchmarkV1RunnerTests.cs), [`BenchmarkV1ThresholdBandWorkloadTests.cs`](Tests/BenchmarkV1ThresholdBandWorkloadTests.cs), [`BenchmarkV1DebtShareDilutionWorkloadTests.cs`](Tests/BenchmarkV1DebtShareDilutionWorkloadTests.cs), [`BenchmarkV1JsonTests.cs`](Tests/BenchmarkV1JsonTests.cs) |
