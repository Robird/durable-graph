...
| Evaluator raw metrics | Explicit Commit boundaries derive total physical writes, per-Commit peak, maximum Current tail, and final cold-head unique full-Frame reads; fresh-file headers and OVD-only reads are covered. | Realized accepted work only; this does not prove a closed horizon, score a rejection, or define a product evaluator. [`EVALUATOR-V1.md`](EVALUATOR-V1.md), [`EvaluatorRawMetricTests.cs`](Tests/EvaluatorRawMetricTests.cs) |
