...
than a target/decision cross-product. Corpus revision 4 runs no-migration, paced, Adaptive
`(3,5%)`, and Adaptive `(4,4%)` over each of three frozen workloads. The first two inputs
retain their earlier parameter-masking results. The third is a canonical hot-chain reset
plus co-resident-expiry/Frame-unpin discriminator: it produces three incomparable unique
raw vectors, with paced exactly tied to Adaptive `(4,4%)`. This is bounded matched Pareto
evidence, not a tuned default, pure rotation-threshold fixture, or general policy ranking.
...
| Adaptive read-amplification/Base-budget v0 | Two payload-proxy parameters select Base/Delta, reserve one old-A progress action per Stay, and trigger Rotate below a strict debt share. Causal witnesses distinguish parameter masking, a local W/R trade, and target timing. In corpus revision 4, the canonical hot-chain/frame-unpin workload separates `(3,5%)` from `(4,4%)`; paced ties `(4,4%)`, and all three unique vectors are incomparable. | Payload heuristic and bounded synthetic evidence only—not a pure `E/G` rotation-band fixture, encoded/physical byte authority, a fifth score, tuned defaults, steady state, or a general winner. [`READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md`](READ-AMPLIFICATION-BASE-BUDGET-POLICY-V0.md), [`BenchmarkV1ThresholdBandWorkloadTests.cs`](Tests/BenchmarkV1ThresholdBandWorkloadTests.cs), [`ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyThresholdBandTests.cs), [`ReadAmplificationBaseBudgetPolicyTargetBandTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyTargetBandTests.cs), [`ReadAmplificationBaseBudgetPolicyCapacityTests.cs`](Tests/ReadAmplificationBaseBudgetPolicyCapacityTests.cs) |
...
| Benchmark-v1 matched consumer | Four strategies from the independent Baselines assembly share each frozen fixture/trace/horizon through the Arena whole-run contract. Revision 4 has 12 admitted cases over three workloads; the previous 8 W/P/F/R vectors and trace hashes remain exact while the expanded manifest/report receive new canonical hashes. | This is an internal-track certified-product proof over three bounded workloads, not a general untrusted-artifact judge or full frontier; single-A-Frame bootstrap, writer-only JSON, no scalar score/parser/CLI. [`EVALUATOR-V1.md`](EVALUATOR-V1.md), [`StrategyArenaContractTests.cs`](Tests/StrategyArenaContractTests.cs), [`BenchmarkV1RunnerTests.cs`](Tests/BenchmarkV1RunnerTests.cs), [`BenchmarkV1ThresholdBandWorkloadTests.cs`](Tests/BenchmarkV1ThresholdBandWorkloadTests.cs), [`BenchmarkV1JsonTests.cs`](Tests/BenchmarkV1JsonTests.cs) |
