namespace Atelia.DurableGraph.StateStore.Serialization;

/// <summary>
/// Owns the change decision and encoded body produced by one Delta preparation.
/// The payload can be reused after representation selection without re-encoding.
/// </summary>
/// <remarks>
/// The producer supplies a consistent decision and payload for the same frozen
/// prior/current pair. This container does not validate the body format, Schema,
/// or prior identity. An unchanged result can contain a nonempty zero bitmap.
/// </remarks>
public sealed class PreparedDelta {
    private readonly byte[] _payload;

    /// <summary>
    /// Copies the payload. The input must remain stable during this call;
    /// subsequent changes to its backing storage cannot change this result.
    /// </summary>
    public PreparedDelta(bool hasChanges, ReadOnlySpan<byte> payload) {
        HasChanges = hasChanges;
        _payload = payload.ToArray();
    }

    public bool HasChanges { get; }
    public ReadOnlySpan<byte> Payload => _payload;
}
